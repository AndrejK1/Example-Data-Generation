...
public class DataGenerationService {
    ...
    public void generateData() {
        // customer params
        long customerCount = 40;

        // product params
        long productCount = 50;
        int productCategories = 10;
        int maxProductStock = 5;
        BigDecimal minProductPrice = BigDecimal.ONE;
        BigDecimal maxProductPrice = BigDecimal.valueOf(50.5);

        // order params
        long orderCount = 1500; // parent orders (real order items rows will be greater)
        ...
        return LongStream.range(startValue, startValue + amount)
                .mapToObj(id -> {
                    ...
                    return new Product(
                            ...
                            available,
                            "http://market/products/" + id + "/thumbnail",
                            "Very good product " + id,
                            round(exTax),
                            round(exTax.add(minPrice.multiply(BigDecimal.valueOf(0.1)))),
                    ...
                    return randomElements(products, r.nextInt(maxLines) + 1L)
                            ...
                            .map(product -> {
                                ...
                                return new Order(
                                        ...
                                        UUID.randomUUID().toString(),
                                        Math.random() > 0.2 ?
                                                product.getListPriceExTax().divide(BigDecimal.TEN, 2, BigDecimal.ROUND_HALF_UP) :
                                                BigDecimal.ZERO,
                                        product.getListPriceExTax(),
                                        product.getPrice(),
                                        timestamp,
                                        timestamp,
                                        "North Road",
